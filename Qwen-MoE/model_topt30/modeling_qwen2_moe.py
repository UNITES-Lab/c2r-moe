...
class Qwen2MoeSparseMoeBlock(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.collaborative_list = config.collaborative_list
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        assert self.top_t >= self.top_k - 1
        top1_idx = torch.argmax(routing_weights, dim=-1)
        top1_weight = torch.gather(routing_weights, 1, top1_idx.view(-1, 1))
        collaborative_indices = torch.tensor(self.collaborative_list[self.layer_idx], device=top1_idx.device)
        collaborative_indices = collaborative_indices[top1_idx]
        collaborative_indices = collaborative_indices[:, :self.top_t]
        collaborative_scores = torch.gather(routing_weights, 1, collaborative_indices)
        routing_weights, selected_experts = collaborative_scores.topk(self.top_k - 1, dim=-1, sorted=False)
        selected_experts = torch.gather(collaborative_indices, 1, selected_experts)
        selected_experts = torch.cat([top1_idx.view(-1, 1), selected_experts], dim=-1)
        routing_weights = torch.cat([top1_weight.view(-1, 1), routing_weights], dim=-1)
        self.top_k_indices = selected_experts
