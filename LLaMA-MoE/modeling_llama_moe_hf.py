...
class TopKBalancedNoisyGate(nn.Module):
    def __init__(
        ...
        self.top_t = top_t
        self.collaborative_list = collaborative_list
    ...
    def forward(self, x):
        ...
        collaborative_indices = torch.tensor(self.collaborative_list[self.layer_index], device=top_indices.device)
        collaborative_indices = collaborative_indices[top_indices.view(-1)]
        if self.top_t > 1:
            collaborative_indices = collaborative_indices[:, :self.top_t]
            collaborative_scores = torch.gather(logits, 1, collaborative_indices)
            collaborative_scores, collaborative_indices_ = collaborative_scores.topk(self.num_selects, dim=1)
            collaborative_indices = torch.gather(collaborative_indices, 1, collaborative_indices_)
            top_indices = torch.cat([top_indices, collaborative_indices], dim=1)
            top_logits = torch.cat([top_logits, collaborative_scores], dim=1)
            top_k_indices = top_indices[:, :self.num_selects]
            top_k_logits = top_logits[:, :self.num_selects]
        else:
            collaborative_indices = collaborative_indices[:, :2]
            top_indices = torch.cat([top_indices, collaborative_indices], dim=1)
    ...
    def _create_gate(self, **kwargs):
        ...
        if self.gate_type == "TopKBalancedNoisyGate":  # noisy gate
            self.gate = TopKBalancedNoisyGate(
                ...
                top_t=kwargs.get("top_t", 1),
                collaborative_list=kwargs.get("collaborative_list", None),
            )
        else:
            raise NotImplementedError
    ...
    def __init__(self, config: LlamaMoEConfig, layer_index):
        ...
        self.mlp = LinearGLUMoELayer(
            ...
            **calculator_config,
            ...
            top_t=config.top_t,
            collaborative_list=config.collaborative_list,
        )
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if layer_outputs[1] is not None:
                if isinstance(balance_loss, torch.Tensor):
                    balance_loss += layer_outputs[1].to(balance_loss.device)
                else:
